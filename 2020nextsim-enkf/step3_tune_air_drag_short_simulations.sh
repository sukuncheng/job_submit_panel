...
function WaitforTaskFinish(){
    ...
    while [[ $XPID -gt 20 ]]; do   #XPID0=$(squeue -u chengsukun | grep -o chengsuk |wc -l) 
        sleep 60
...
>nohup.out  
    ...
    tduration=22   # number of DA cycles. 
    ENSSIZE=1      # ensemble size  
    block=1        # number of forecasts in a job
    jobsize=$((${ENSSIZE}/${block})) #number of nodes requested 

    air_drag_coef=(0.0004 0.0008 0.0012 0.0014 0.0016 0.0018 0.002 0.0024 0.0028 0.0032)
    #
    OUTPUT_DIR=${simulations}/test_tune_air_drag_${start_date}_${duration}days_x_${tduration}cycles
...
for (( iperiod=1; iperiod<=${tduration}; iperiod++ )); do
    ...
    # a. setting config files
        sed -i "s|^dynamics-type=.*$|dynamics-type=bbm|g"  ${JOB_SETUP_DIR}/nextsim.cfg
    ...
    # b. create work path
        ...
        mkdir -p ${member_dir}
        cd $member_dir
        grep -q -s "Simulation done" ${member_dir}/task.log && continue
