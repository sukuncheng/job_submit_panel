...
# C_lab=6.8465e+6*linspace(2.5,50,20)/25 (Pa)
C_lab=( 684650     1369300     2053950     2738600     3423250    \
       4107900     4792550     5477200     6161850     6846500    \
       7531150     8215800     8900450     9585100    10269750    \
      10954400    11639050    12323700    13008350    13693000)
Output_dir=/neXtSIM_test10_10_winter_step5_cohesion_perturbation
#############################################################################
for (( i_date=1; i_date<= ${#dir_list[@]}; i_date++ )); do
for (( i_ens =1; i_ens <= ${#C_lab[@]}; i_ens++ )); do #for (( i_ens =1; i_ens <= ${#alea_factor[@]}; i_ens++ )); do
...
# #/`printf "ENS%.2d" "$i_ens"`
for (( i_ens =1; i_ens <= ${#C_lab[@]}; i_ens++ )); do
for (( i_date=1; i_date<= ${#dir_list[@]}; i_date++ )); do
      # # make the output directories L2
