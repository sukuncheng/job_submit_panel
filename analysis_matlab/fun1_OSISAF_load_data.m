%% load_OSISAF_data
function fun1_OSISAF_load_data(simul_dir,filename)        
    load(filename)
    %
    ...
    for iperiod = 1:N_periods
        ...
        for ie = 1:Ne
            data_dir = [simul_dir '/date' num2str(iperiod) '/ENS' num2str(ie)];    % set file directory        
            [osisaf_model(iperiod).ensemble(ie).short_term,osisaf_obs(iperiod).ensemble(ie).short_term] = ...
                            fun_get_simulated_observed_OSISAF_ice_drift(data_dir,mnt_dir,dates);               
        end
    end
...
%
    ...
    %
        readin_file = [data_dir '/OSISAF_' TEMP '.nc'] ;
...
%     2008 file has a prefix "ice-drift_"
    filename = [ mnt_dir '/OSISAF_ice_drift/' YEAR '/' MONTH '/' ...
