...
% mbs_in  -- (POC)﻿The probability of finding a drifting object inside the search area
    ...
    if(buoy_type==1)
        ...
        Obs_pos_x = Pt_pos_x(1:Ne:end,:); % 21-th ensemble member is not perturbed, thus, as a reference
        Obs_pos_y = Pt_pos_y(1:Ne:end,:);
    end
    ...
    for j = 1:N_buoy     
        for it = 2:N_t               
            ...
            if R>5  % smoothing R: treshold = 5; depending outliers shown by plot(R)igma_b_para<0.003 && sigma_b_perp<0.003 || length(id)<11 || length(id)<0.5*Ne_include
                R = nan;
                option2(j,it).area = nan;
                option2(j,it).POC = nan;
            else
            ...
            try                 
                ...
                option3(j,it).area = option3(j,it).ellipse.area;
                ...
                option3(j,it).area = 0;  % ellipse includes (coorinates, axis:a,b, anisotropy:a/b, area:\pi*a*b)
                option3(j,it).ellipse.anisotropy = 0;   
                option3(j,it).POC = 0;
            end         
        end               
    end
    ...
    % save IABP observation related variables
    features.e = e;
