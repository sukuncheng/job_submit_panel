function [] = main_enkf_diagnc()
    ...
    function statistics = fun_process_enkf_diag(enkf_dir)
        ...
        if strcmp(Exp_ID,'Spinup')
            filename = [enkf_dir '/size40_I1_L300_R2_K2_DAsic/enkf_diag.nc'];
        else
    ...
    %     statistics(6) = nanmean(reshape(pnlobs,1,[]));
        ...
        subplot(231); fun_geo_plot(lon,lat, dfs,[datestr(title_date) ' dfs'],unit); %caxis([0 18])
        subplot(232); fun_geo_plot(lon,lat, srf,' srf',unit); %caxis([0 3])
