...
for (( iperiod=1; iperiod<=${tduration}; iperiod++ )); do
    ...
    for (( i=1; i<=${ENSSIZE}; i++ )); do
        ...
        if [ $iperiod -eq 1 ]; then
            ...
            #
            restart_source=/cluster/work/users/chengsukun/src/IO_nextsim/ensemble_forecasts_2019-09-03_7days_x_5cycles_memsize100/date6
            #
            # cd   ${restart_source}/filter/I${INFLATION}_L${LOCRAD}_R${RFACTOR}_K${KFACTOR}
            # rm -f ${memname}.nc.analysis   //should be done ahead
            # cdo merge ${restart_path}/reference_grid.nc   $(printf "mem%.3d" $i).nc.analysis  ${memname}.nc.analysis       
            ln -sf ${restart_source}/filter/I${INFLATION}_L${LOCRAD}_R${RFACTOR}_K${KFACTOR}/${memname}.nc.analysis         ${restart_path}/${memname}.nc.analysis   
    ...
    # 0. create files strucure, copy and modify configuration files inside
        cp ${JOB_SETUP_DIR}/{part0_forecast_from_reanalysis.sh,part1_create_file_system.sh}  ${ENSPATH} 
