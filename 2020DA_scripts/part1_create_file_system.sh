...
#1. prepare forecast files
    for (( mem=1; mem<=${ESIZE}; mem++ )); do
        MEMPATH=${ENSPATH}/mem$(printf "%03d" $mem)
	mkdir -p ${MEMPATH}  
        # pseudo2D.nml      
        ...
        sed -e "s;^duration=.*$;duration="${duration}";g" \
            ...
            ${JOB_SETUP_DIR}/nextsim.cfg > ${MEMPATH}/nextsim.cfg
    done
...
#2. prepare analysis files
    ...
    cp -r ${JOB_SETUP_DIR}/enkf_cfg .  #from ${NEXTSIMDIR}/modules/enkf/enkf-c/cfg/* # except stats.prm and enoi.prm
