...
sed -i "s|^export USE_ENSEMBLE.*$|export USE_ENSEMBLE=|g;" ${ENV_FILE}  # NEED TO RECOMPILE neXtSIM
cd ~/src/nextsim
make fresh -j8
...
##-------  Confirm working,data,ouput directories --------
    ...
    ASR_air_drag=(0.0016)
    OUTPUT_DIR=${simulations}/test_deterministic_${time_init}_${duration}days
