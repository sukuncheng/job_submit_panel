...
########################## changes  #########################################
Output_dir=/neXtSIM_test12_07_winter_step3_air_drag_coefficient
...
ASR_air_drag=( 0.003 0.0035 0.004 0.0045 0.005 0.0055 0.006 0.0065 0.007 0.0075 0.008 )
Ne=${#ASR_air_drag[@]}
# #############################################################################
...
#              ######################>>>>>>>>>>>>>>>>>>
#             sed -i "s|^ASR_quad_drag_coef_air.*$|ASR_quad_drag_coef_air=${ASR_air_drag[$i_ens-1]}|g" $config  
#             sed -i "s|^use_moorings.*$|use_moorings=false|g" $config
#             sed -i "s|^use_iabp_drifters.*$|use_iabp_drifters=false|g" $config
#             sed -i "s|^use_equally_spaced_drifters.*$|use_equally_spaced_drifters=false|g" $config
#             ######################<<<<<<<<<<<<<<<<<<<
