...
for (( i_ens =1; i_ens <=$Ne; i_ens++ )); do
      ...
      ENSdir=$IO_nextsim/neXtSIM_test09_06_winter_step4b_wind_perturbation_vwnd_3_exp2/date$i_date/ENS$i_ens
      ...
      echo $XPID    
      while [[ $XPID -ge 6 ]]; do # set the maximum of simultaneous running jobs, don't have to be Ne
            sleep 20
            job_list=$(squeue -u chengsukun)
            XPID=$(grep -o chengsuk <<<$job_list |wc -l)  # number of current running jobs
      done          
