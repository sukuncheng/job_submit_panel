...
#
for (( i_alea=1; i_alea <= ${#alea_factor[@]}; i_alea++ )); do
for (( i_date=1; i_date<= ${#dir_list[@]}; i_date++ )); do # ${#dir_list[@]}
for (( i_ens =1; i_ens <=$Ne; i_ens++ )); do
      # make the output directories L2
      ENSdir=$IO_nextsim/neXtSIM_test09_28_winter_step6_wind_cohesion/alea_factor$i_alea/date$i_date/ENS$i_ens
      
      # modify config files and send to member directories
      sed -i "s|^alea_factor.*$|alea_factor=${alea_factor[$i_alea-1]}|g" $config      
